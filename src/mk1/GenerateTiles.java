...
public class GenerateTiles 
{
	...
	// This generate method works by generating a field of grass, and adding other tiles on top. Rocks
	// are randomly placed, the river is generated using the "wandering drunk" method, and sand is placed on any grass
	// tile bordering river.  It is sloppy, but it works lmao.
	void generate()
	{
	...
	void generateRocks(int numRocks)
	{
		...
		for(int i = 0; i < numRocks; i++)
		{
			...
			tiles[newRock.x][newRock.y].type = "ROCK";
		}
	}
	...
	void generateRiver(int rivers)
	{
		...
		for(int i = 0; i < rivers; i++)
		{
			...
			source.x = 0;
			tiles[source.x][source.y].type = "WATER";
			...
			while (source.x < x - 1)
			{
				...
				if(direction == 0)
				{
					...
					tiles[source.x + 1][source.y].type = "WATER"; // down
					tiles[source.x + 1][source.y + 1].type = "WATER"; // right
					source.x += 1;
					source.y += 1;
				...
				else
				{
					...
					// Down left
					tiles[source.x + 1][source.y].type = "WATER"; // down
					tiles[source.x + 1][source.y - 1].type = "WATER"; // left
					source.x += 1;
					source.y -= 1;
					System.out.println("New source tile at" + source.x + "," + source.y);
				}
	...
	void generateSand()
	{
		...
		for (int i = 0; i < x; i++)
		{
			for (int j = 0; j < y; j++)
			{
				...
				if(i - 1 > 0 && tiles[i - 1][j].type == "WATER") // Up
					tiles[i][j].type = "SAND";
				else if(j + 1 < y && tiles[i][j + 1].type == "WATER") // Right
					tiles[i][j].type = "SAND";
				else if(i + 1 < x && tiles[i + 1][j].type == "WATER") // Down
					tiles[i][j].type = "SAND";
				else if(j - 1 > 0 && tiles[i][j - 1].type == "WATER") // Left
					tiles[i][j].type = "SAND";
